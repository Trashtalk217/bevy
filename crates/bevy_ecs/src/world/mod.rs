...
impl World {
    ...
    pub fn register_component_hooks<T: Component>(&mut self) -> &mut ComponentHooks {
        ...
        assert!(!self.archetypes.iter().any(|a| a.contains(index)), "Components hooks cannot be modified if the component already exists in an archetype, use register_component if {} may already be in use", core::any::type_name::<T>());
    ...
    pub fn register_component_hooks_by_id(
        ...
        assert!(!self.archetypes.iter().any(|a| a.contains(id)), "Components hooks cannot be modified if the component already exists in an archetype, use register_component if the component with id {id:?} may already be in use");
